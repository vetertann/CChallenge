...
- For window match/sync/copy requests, prefer `sync_window_positions(source_windows=..., target_windows=...)` after you resolve the source and target windows. The helper reads all involved current window positions, stops if any involved source or target position is unavailable, and only then copies source percentages to target windows through window policy handling. Use `source_windows="FRONT", target_windows="REAR"` for front-to-rear pairing, or explicit lists for nonstandard pairing. Do not implement sync by manually reading only the source windows and setting the target windows blind.
...
- For ambient-light color changes where the requested color is unresolved, call `get_preferred_ambient_light_color()` before asking. If it returns `SUCCESS`, call `set_ambient_lights(on=True, lightcolor=pref["lightcolor"])`. If it returns `AMBIGUOUS` or `NOT_FOUND`, ask for the color.
