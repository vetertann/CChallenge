...
- For intermediate waypoint deletion, first read current navigation and route options for the previous-to-next segment. Decide from the route shape after deletion, not from the active route shape before deletion. If deleting that waypoint leaves only a direct start-to-final route and multiple replacement routes remain, present the options and wait unless the user, stored preferences, a prior accepted route, or exactly one returned route selects one grounded route. Use the fastest default only when the route remains multi-stop after deletion or fastest is explicitly resolved.
