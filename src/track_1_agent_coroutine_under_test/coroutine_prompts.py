...
- Missing capability outranks clarification and perfect drafting. For an explicit requested side effect such as a vehicle setting, phone call, or email, call the relevant wrapper as soon as the public required arguments you know are grounded. Do not manually promise the action or manually ask confirmation; the wrapper path reports missing tools or parameters safely. If a concrete side effect still needs a user value before its wrapper can be called, use `ask_for_missing_action_value(...)` instead of `respond(...)` so the runtime checks the live tool/parameter surface before asking. Before using it, first check policy, explicit user choices, stored preferences, current state, and helper defaults; do not ask for a value that one of those sources already resolves.
...
def render_workspace_helpers() -> str:
    text = (
        ...
        "  Built-in response helper, not a direct evaluator tool. Use only when one concrete requested side effect still needs a user value before its wrapper can be called, such as a heating level, window percentage, phone number, or email address. `requirements` must name wrapped evaluator tools and parameters, not workspace helpers. It checks only the current task's tool/parameter surface, calls no action tool, and either asks `question` or reports the missing capability. Do not use it for broad comfort options, route/POI choices, lookup/search/read tools, or facts that can be read first. Do not use it when stored preferences already resolve the missing value.\n"
        ...
        "  Built-in read-only helper, not a direct evaluator tool. Reads already preflighted climate preferences from scratchpad and returns a unique Celsius temperature for the requested zone when preferences resolve one; otherwise returns `AMBIGUOUS` or `NOT_FOUND`.\n"
    )
    replacements = {
